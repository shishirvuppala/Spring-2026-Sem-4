...
from model import SymmetricMultiHeadAttention
...
def evaluate():
    ...
    try:
        model = SymmetricMultiHeadAttention(d_model=128, num_heads=4).to(device)
        model.eval()
    ...
    print(f"Checking Task 2: Symmetric Attention Implementation...")
    print("-" * 60)
