...
class LinearRegressionTree:
    ...
    def _find_best_split(self, X, y, parent_loss, parent_stats):
        ...
        for j in range(d):
            ...
            for i in range(N - 1):
                ...
                if n_L > 1:
                    alpha_L = n_L / (n_L - 1)          
                    Sxx_L += alpha_L * np.outer(dx_L, dx_L)    
                    Sxy_L += alpha_L * np.outer(dx_L, dy_L)   
                    Syy_L += alpha_L * dy_L ** 2               
    ...
    def _predict_batch(self, X, indices, node, y_pred):
        ...
        feat_vals = X[node.feature, indices]
